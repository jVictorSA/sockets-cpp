...
void ConcatenandoStr(char *name, char *msg){
    int len_name = strlen(name);
    int len_msg = strlen(msg);
    cout << "-"<<len_name<<"-"<<len_msg<<endl;
    
    //Introduzindo dois pontos e espaço:
    name[len_name] = (char)':';
    name[len_name+1] = (char)' ';

    //Adicionando mensagem:
    int acc = 0;
    for (int i=len_name+2; i<len_name+1+len_msg; i++){
        name[i] = (char)msg[acc];
        acc+=1;
    }
}
...
int __cdecl main(int argc, char **argv){
    ...
    char mensagem[DEFAULT_BUFLEN];
    mensagem[0] = '0';
    while (mensagem[0] == '0'){
        cout << "Digite 1 para sair do Chat Livre ou envie uma mensagem:"<<endl;
        cin.getline(mensagem, DEFAULT_BUFLEN);
        ...
        ConcatenandoStr(buffer_nome, mensagem);
        resultado = send(ConnectSocket,
        buffer_nome,
        (int)strlen(buffer_nome), //PODE SER NECESSÁRIA A LIB string.h, SE NÃO A FUNÇÃO DEVE ESTA EM OUTROS CABEALHOS JÁ DEFINIDOS
        0);
        if (resultado == SOCKET_ERROR){
